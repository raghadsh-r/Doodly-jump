...
def salem(dark,char):
    ...
    white = (255,255,255)
    fps=90
    bg_photo='Space/space.jpg'
    gmover_photo='Space/spacegameover.jpg'
    playercharacter='Space/player.png'
    platform_img = pygame.image.load("Space/spacestep.png")
    ...
    def update_player(p_y):
        nonlocal jump
        nonlocal y_change
        jump_heghit =8 #11.0*(score**4+2)/((score**4+2.3))
        gravity=.2
        if jump:
            jumpsound.play()
    ...
    def update_paltforms(plat,y_pos,change):
        ...
        if y_pos<300 and change<0:
            for i in range(len(plat)):
                if plat[i][1] < 700:
                    plat[i][1]-=change#*((score**4+2)/((score**4+2.2)))
        else :
            pass
        ...
        for i in range(len(plat)):
            if plat[i][1]>660 and y_pos<300 :
                plat[i]=[randint(370,1000)%440,randint(-60,-20),randint(40,130),randint(16,23)] 
                score+=1
    ...
    while running==True:
        ...
        if game_over:
            ...
            screen.blit(superJUMPtext, (0, 14))
